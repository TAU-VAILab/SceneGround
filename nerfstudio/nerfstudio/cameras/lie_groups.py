...
def log_map_SO3xR3(transform_matrix: Float[Tensor, "b 3 4"]) -> Float[Tensor, "b 6"]:
    """Compute the logarithmic map of the direct product group `SO(3) x R^3`.

    This is the inverse of `exp_map_SO3xR3`.

    Args:
        transform_matrix: Transformation matrices of shape (b, 3, 4), where
                          the first 3x3 block is the rotation matrix R, and
                          the last column is the translation vector t.
    Returns:
        Tangent vector; length-3 translations, followed by an `so(3)` tangent vector.
    """
    R = transform_matrix[:, :3, :3]  # Rotation matrix
    t = transform_matrix[:, :3, 3]  # Translation vector

    # Compute the log map for SO(3)
    trace_R = R[:, 0, 0] + R[:, 1, 1] + R[:, 2, 2]
    theta = torch.acos(torch.clamp((trace_R - 1) / 2, -1, 1))  # Rotation angle

    log_rot = torch.zeros((R.shape[0], 3), dtype=R.dtype, device=R.device)
    small_theta = theta < 1e-4

    if not small_theta.any():
        skew_log_rot = (R - R.transpose(1, 2)) / (2 * torch.sin(theta)[:, None, None])
        log_rot[:, 0] = skew_log_rot[:, 2, 1]
        log_rot[:, 1] = skew_log_rot[:, 0, 2]
        log_rot[:, 2] = skew_log_rot[:, 1, 0]
        log_rot *= theta[:, None]  # Scale by theta
    else:
        # Near zero, use a series expansion
        delta_R = R - torch.eye(3, device=R.device, dtype=R.dtype)[None]
        log_rot[:, 0] = 0.5 * delta_R[:, 2, 1]
        log_rot[:, 1] = 0.5 * delta_R[:, 0, 2]
        log_rot[:, 2] = 0.5 * delta_R[:, 1, 0]

    # Combine translation and log rotation
    tangent_vector = torch.zeros((transform_matrix.shape[0], 6), dtype=R.dtype, device=R.device)
    tangent_vector[:, :3] = t
    tangent_vector[:, 3:] = log_rot
