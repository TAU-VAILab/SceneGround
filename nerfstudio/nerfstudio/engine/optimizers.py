...
class Optimizers:
    ...
    def load_optimizers(self, loaded_state: Dict[str, Any]) -> None:
        ...
        for k, v in loaded_state.items():
            print(f"Loading optimizer state for {k}")
            if (k != "features" and k != "features_speedup") or k in self.optimizers:
                self.optimizers[k].load_state_dict(v)
            else:
                print(f"Skipping loading optimizer state for {k}")
    ...
    def load_schedulers(self, loaded_state: Dict[str, Any]) -> None:
        ...
        for k, v in loaded_state.items():
            if (k != "features" and k != "features_speedup") or k in self.optimizers:
                ...
                print(f"Skipping loading scheduler state for {k}")
