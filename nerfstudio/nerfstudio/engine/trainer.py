...
class TrainerConfig(ExperimentConfig):
    ...
    regitsration: bool = False
    """Whether to train the registration matrix"""
    cathedral_number: Optional[int] = None
    colmap_number: Optional[int] = None
...
class Trainer:
    ...
    @property
    def cathedral_transform_paths(self) -> Optional[List[Path]]:
        if self.config.cathedral_number is not None:
            paths = []
            paths.append(self.base_dir / "transform.pt")
            paths.append(Path("transforms") / str(self.config.cathedral_number) / str(self.config.colmap_number) / f"{self.config.transform_dir}_transform.pt")
            return paths 
        return None

    @property
    def cathedral_per_image_registration_path(self) -> Path:
        if self.config.cathedral_number is not None:
            return Path("transforms") / str(self.config.cathedral_number) / str(self.config.colmap_number) / f"{self.config.transform_dir}_all_registration_transform.pt"
        return None
    
    def save_registration(self) -> None:
        image_file_names = self.pipeline.datamanager.train_dataparser_outputs.image_filenames
        image_rename_maps = self.pipeline.datamanager.train_dataparser_outputs.metadata["image_rename_map"]
        image_names_to_save = []
        for name in image_file_names:
            save_name = image_rename_maps[name.name]
            image_names_to_save.append(save_name)
        output = {}
        
        for image_idx in range(self.pipeline.model.registration.shape[0]):
            registration =  exp_map_SO3xR3(self.pipeline.model.registration[image_idx, :,:-1])
            only_registration = torch.cat((registration.squeeze(), torch.tensor([[0, 0, 0, 1]], device=self.device))).unsqueeze(0)
            scale = self.pipeline.model.registration[image_idx, 0, -1]
            image_name = image_names_to_save[image_idx]
            output[image_name] = {"registration": only_registration, "scale": scale}
        path = self.cathedral_per_image_registration_path
        path.parent.mkdir(parents=True, exist_ok=True)
        torch.save(output, path)

    def save_transform(self) -> None:
        if self.cathedral_transform_paths is not None:
            registration = exp_map_SO3xR3(self.pipeline.model.registration[:,:-1])
            registration = torch.cat((registration.squeeze(), torch.tensor([[0, 0, 0, 1]], device=self.device))).unsqueeze(0)
            scale = self.pipeline.model.registration[0,-1]
            for path in self.cathedral_transform_paths:
                path.parent.mkdir(parents=True, exist_ok=True)
                torch.save({"registration": registration, "scale": scale}, path)
    ...
    def setup(self, test_mode: Literal["test", "val", "inference"] = "val") -> None:
        ...
        # Only if registration is enabled remove all optimizers except registration
        # It should happen after load_checkpoint to load the correct optimizer
        if self.config.regitsration:
    ...
    def setup_optimizers_for_registration(self) -> Optimizers:
        ...
        Returns:
            The optimizers object given the trainer config.
        """
        parameters_to_keep = ["registration"]
        # , "opacities"
        optimizer_config = self.config.optimizers.copy()
        param_groups = self.pipeline.get_param_groups()
        ...
        camera_param_groups = {}
        for parameter_to_keep in parameters_to_keep:
            for param in param_groups[parameter_to_keep]:
                param.requires_grad = True

            camera_optimizer_config[parameter_to_keep] = optimizer_config[parameter_to_keep]
            camera_param_groups[parameter_to_keep] = param_groups[parameter_to_keep]
        return Optimizers(camera_optimizer_config, camera_param_groups)
    ...
    def train(self) -> None:
        ...
        with TimeWriter(writer, EventName.TOTAL_TRAIN_TIME):
            ...
            for step in range(self._start_step, self._start_step + num_iterations):
                ...
                if self.config.regitsration:
                    if hasattr(self.pipeline.model, "image_filenames"):
                        self.pipeline.model.image_filenames = self.pipeline.datamanager.train_dataparser_outputs.image_filenames
                    if hasattr(self.pipeline.model, "image_rename_map"):
                        self.pipeline.model.image_rename_map = self.pipeline.datamanager.train_dataparser_outputs.metadata["image_rename_map"]
                    original_registration = self.pipeline.datamanager.train_dataparser_outputs.metadata["registration_matrix"]
                    registration = exp_map_SO3xR3(self.pipeline.model.registration[:,:-1])
                    registration = torch.cat((registration.squeeze(), torch.tensor([[0, 0, 0, 1]], device=self.device))).unsqueeze(0)
                    original_registration_position, original_registration_angles = self.transformation_to_translation_angle(original_registration.cpu().numpy())
                    registration_position, registration_angles = self.transformation_to_translation_angle(registration.squeeze().detach().cpu().numpy())
                    original_position_mse = torch.nn.functional.mse_loss(torch.from_numpy(original_registration_position), torch.from_numpy(registration_position))
                    original_angle_mse = torch.nn.functional.mse_loss(torch.from_numpy(original_registration_angles), torch.from_numpy(registration_angles))
                    original_registration_mse = torch.nn.functional.mse_loss(original_registration.cpu(), registration.squeeze().detach().cpu())

                # a batch of train rays
                if step_check(step, self.config.logging.steps_per_log, run_at_zero=True):
                    if self.config.regitsration:
                        writer.put_scalar(name="original_position_mse", scalar=original_position_mse, step=step)
                        writer.put_scalar(name="original_angle_mse", scalar=original_angle_mse, step=step)
                        writer.put_scalar(name="original_registration_mse", scalar=original_registration_mse, step=step)
                        writer.put_scalar(name="Scale", scalar=self.pipeline.model.registration[0,-1], step=step)
                ...
                # registration = exp_map_SO3xR3(self.pipeline.model.registration[:,:-1])
                if step_check(step, self.config.steps_per_save / 10):
                    self.save_transform()
    ...
    def transformation_to_translation_angle(self, transformation):
        translation = transformation[:3, 3]
        rotation = transformation[:3, :3]
        phi = np.arctan2(rotation[1, 0], rotation[0, 0])
        theta = np.arctan2(-rotation[2, 0], np.sqrt(rotation[2, 1] ** 2 + rotation[2, 2] ** 2))
    ...
    def train_iteration(self, step: int) -> TRAIN_INTERATION_OUTPUT:
        ...
        with torch.autocast(device_type=cpu_or_cuda_str, enabled=self.mixed_precision):
            ...
            should_account_loss = "should_not_account_loss" not in loss_dict
            loss = functools.reduce(torch.add, loss_dict.values())
        if should_account_loss:
