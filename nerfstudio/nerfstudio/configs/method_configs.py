...
method_configs["register_splatfacto"] = TrainerConfig(
    method_name="splatfacto",
    steps_per_eval_image=100,
    steps_per_eval_batch=0,
    steps_per_save=2000,
    ...
    pipeline=VanillaPipelineConfig(
        datamanager=FullImageDatamanagerConfig(
            dataparser=NerfstudioDataParserConfig(load_3D_points=True, registration=True),
            cache_images_type="uint8",
        ),
        model=SplatfactoModelConfig(registration=True, stop_split_at=10),
    ),
    optimizers={
        "means": {
            "optimizer": AdamOptimizerConfig(lr=1.6e-4, eps=1e-15),
        ...
        "registration": {
            # Good optimizer for both scenes, scheduler is a must because it wont converge otherwise
            "optimizer": AdamOptimizerConfig(lr=1e-3, eps=1e-8),
            "scheduler": None,
        },
        # Good optimized for cathedral
        # "optimizer": AdamOptimizerConfig(lr=1e-3, eps=1e-8),
...
method_configs["splatfacto_features"] = TrainerConfig(
    method_name="splatfacto",
    steps_per_eval_image=100,
    steps_per_eval_batch=0,
    steps_per_save=2000,
    ...
    pipeline=VanillaPipelineConfig(
        ...
        model=SplatfactoModelConfig(optimize_features=True),
    ),
    optimizers={
        "means": {
            "optimizer": AdamOptimizerConfig(lr=1.6e-4, eps=1e-15),
...
method_configs["register_splatfacto_features"] = TrainerConfig(
    method_name="splatfacto",
    steps_per_eval_image=100,
    steps_per_eval_batch=0,
    steps_per_save=2000,
    ...
    pipeline=VanillaPipelineConfig(
        ...
        # Do not split gaussians when registering.
        model=SplatfactoModelConfig(registration=True, optimize_features=True, stop_split_at=10),
    ),
    optimizers={
        "means": {
            "optimizer": AdamOptimizerConfig(lr=1.6e-4, eps=1e-15),
