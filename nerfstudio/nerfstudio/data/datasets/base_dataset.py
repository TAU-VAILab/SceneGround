...
class InputDataset(Dataset):
    ...
    def __init__(self, dataparser_outputs: DataparserOutputs, scale_factor: float = 1.0, output_features=False, features_type=None, mask_type:Optional[str] = None):
        super().__init__()
    ...
    def get_features(self, image_idx: int):
        # gerrard-hall/images_4/frame_00010.JPG
        image_path = Path(self._dataparser_outputs.image_filenames[image_idx])
        feature_path = image_path.parents[1] / f"images_{self.features_type}_features" / f"{image_path.name}.pt" 
        # Chane it to [H, W, F]
        # Map it to cpu because Dataset is loaded in cpu
        return torch.load(feature_path, map_location="cpu").squeeze(0).permute(1, 2, 0)

    def get_loss_mask(self, image_idx: int):
        image_path = Path(self._dataparser_outputs.image_filenames[image_idx])
        mask_path = image_path.parents[1] / f"images_{self.mask_type}_masks" / f"{image_path.name}.pt" 
    ...
    def get_data(self, image_idx: int, image_type: Literal["uint8", "float32"] = "float32") -> Dict:
        ...
        if self.output_features:
            try:
                features = self.get_features(image_idx)
            except FileNotFoundError:
                features = None
        else:
            features = None
        if self.mask_type is not None:
            mask = self.get_loss_mask(image_idx)
        else:
            mask = None
        data = {"image_idx": image_idx, "image": image, "features": features, "loss_mask": mask}
