...
class FullImageDatamanager(DataManager, Generic[TDataset]):
    ...
    def _load_images(
        ...
        def undistort_idx(idx: int) -> Dict[str, torch.Tensor]:
            ...
            scaling_factor = data['image'].shape[1] / camera.width.item()
            ...
            if mask is not None:
                data["mask"] = mask
            # Changing the cameras parameters if the camera image size differs from the image size
            dataset.cameras.fx[idx] = float(K[0, 0]) * scaling_factor
            dataset.cameras.fy[idx] = float(K[1, 1]) * scaling_factor
            dataset.cameras.cx[idx] = float(K[0, 2]) * scaling_factor
            dataset.cameras.cy[idx] = float(K[1, 2]) * scaling_factor
        ...
        if cache_images_device == "gpu":
            for cache in undistorted_images:
                ...
                if "features" in cache and cache["features"] is not None:
                    cache["features"] = cache["features"].to(self.device)
    ...
    def create_train_dataset(self) -> TDataset:
        ...
        return self.dataset_type(
            ...
            features_type=self.config.dataparser.feature_type,
            mask_type=self.config.dataparser.mask_type
        )
